...
It costs money + tokens, needs OPENROUTER_API_KEY (the brain is GLM-only via
OpenRouter now), takes a few minutes, and the model's
...
def main() -> int:
    ...
    if not (REPO / ".env").exists():
        # The brain is GLM-only via OpenRouter now (ea84af1); the real analyze.py
        # needs OPENROUTER_API_KEY. Without a .env it must be in the env.
