...
Quiver is an autonomous once-daily stock bot that trades **real money**. A multi-agent
LLM framework (vendored `TradingAgents`, running on DeepSeek) produces a daily Buy/Sell/Hold
signal per ticker. Claude Code — holding the Robinhood MCP — is the *execution orchestrator*:
it feeds broker data to deterministic Python and places exactly the orders that Python returns.
...
split — do not move clamping, sizing, dedup, or halt logic into prose/runbook steps, and do not
let the analysis path read trading limits.

## Current status (2026-05-30)

Built and validated end-to-end; in **dry-run**, pending one live session.

- ✅ 26/26 unit tests; `analyze.py AAPL` ran a full DeepSeek pass → valid signal;
...
1. **`tick.py preflight`** (Python) — gate: kill-switch file, daily-halt flag, market open,
   minutes-since-open, per-day dedup. Returns `proceed` + config + `pending` tickers +
   `unfinalized` orders to reconcile.
2. **`get_portfolio` / `get_equity_positions`** (MCP) — read-only broker snapshot. Auth error
   here aborts the whole tick — never trade on stale auth.
3. **`analyze.py <TICKER>`** (Python → DeepSeek) — slow (~minutes per ticker, 900s timeout).
   One JSON line per ticker. Errors print `"signal":"ERROR"` and are recorded as skips.
4. **`tick.py plan`** (Python) — the deterministic brain: applies the daily-loss halt, maps
   signals to intents, clamps sizing, reserves `ref_id`s. Returns the explicit `orders` list.
5. **`review_equity_order` / `place_equity_order`** (MCP) — execute exactly the returned orders.
   In `dry_run` the orchestrator reviews but never places.
6. **`tick.py commit`** (Python) — finalize each order + write the per-ticker action row.
7. **`tick.py report` → Resend MCP → `tick.py report-commit`** (Python builds + dedups, MCP sends) —
   observability only, strictly best-effort: emails the run's digest once per day. A `report`
   error or a failed send is logged and the tick ends normally — it never blocks trading.
...
The loop wakes hourly and no-ops cheaply until the market is open and the ticker hasn't been
acted on today (the `(trade_date, ticker)` dedup guarantees at most one action per ticker per
day regardless of tick count). When editing `TICK.md` and `tick.py` together, keep them in
lockstep — the runbook describes the exact JSON shapes `plan`/`commit` consume.

## Safety architecture (the parts that matter most)

- **`lib/ledger.py`** — SQLite at `state/ledger.db`, the source of truth across restarts. Three
  guarantees: (1) `ticker_action` PK `(trade_date, ticker)` → at most one action per ticker per
  day; (2) `orders` rows are *reserved before* the broker call and keyed by a UUID `ref_id`, so a
  crash mid-place is recoverable and retries reuse the same `ref_id` (Robinhood dedups by it —
  **never mint a new `ref_id` for an order that might already be sent**); (3) `day_baseline.halted`
  + the `KILL` file make every later tick a no-op until a human clears it.
- **`lib/signals.py`** — pure, unit-tested signal→order mapping and sizing clamps. **Long-only
  policy**: a Sell/Underweight with no position is a no-op (never opens a short). Buy dollars are
  the min of (model size or conservative fallback, per-trade ceiling, remaining daily deploy cap,
  buying_power − buffer, room under per-ticker cap); a non-positive result means skip. Sizing
  never "fails open" to a large amount — unparseable prose falls back to `min(ceiling, 100)`.
...
- `tick.py` — `preflight` / `plan` / `commit` + `report` / `report-commit` subcommands. Every
  subcommand prints one JSON line and catches all exceptions into `{"error": ...}` with exit code 1
  (the orchestrator stops on a plan/commit error — but a `report` error is observability-only and
  must NOT stop the tick).
- `lib/` — `config`, `market`, `ledger`, `signals`, `ds_config`, `notify` (see above).
- `config.yaml` — account, `dry_run`, watchlist, dollar caps, order defaults, model IDs, loop timing,
  `notify` (email digest — off unless `enabled: true`).
- `TICK.md` — the per-tick runbook the orchestrator follows verbatim.
- `vendor/TradingAgents` — the vendored multi-agent framework (gitignored; re-cloned via setup).
- `state/` — `ledger.db`, `tmp/` (plan_input/commit/report scratch), `analyze_logs/`, framework
  `results`/`cache`/`memory`. Gitignored.
